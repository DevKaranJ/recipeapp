class GeneralShoppingListsController < ApplicationController
  ...
  def index
    ...
    @total_items = @missing_foods.sum { |food_name, missing_quantity| missing_quantity }
  ...
  def calculate_missing_foods(user, recipes)
    missing_foods = {}
    ingredients = Ingredient.includes(:food).where(recipe_id: recipes.pluck(:id)).group_by { |ingredient| ingredient.food&.name }
